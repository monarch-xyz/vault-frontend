...
function SystemLogBubble({ log }: { log: LogEntry }) {
  ...
  const getLogIcon = () => {
    if (
      log.type.includes('ANALYSIS') ||
      log.type.includes('analysis') ||
      log.type.includes('report')
    ) {
      ...
      log.type === ActivityType.MARKET_DATA_FETCHED ||
      log.type === ActivityType.VAULT_DATA_FETCHED
    ) {
  ...
  const getBubbleStyle = () => {
    if (log.type.includes('reasoning')) {
      ...
      log.type === ActivityType.AGENT_STARTED ||
      log.type === ActivityType.AGENT_STOPPING
    ) {
  ...
  const getReasoningSummary = () => {
    if (
      log.type === ActivityType.REASONING_STARTED ||
      log.type === ActivityType.REASONING_COMPLETED
    ) {
      return (
        log.data?.summaryText ||
        (log.type === ActivityType.REASONING_STARTED
    ...
    <div className={getBubbleStyle()}>
      <div className="mb-2 flex items-center justify-between">
        ...
        <span
          ...
          title={format(new Date(log.timestamp), 'HH:mm:ss')}
        >
    ...
    <div className="space-y-3">
      {messages.length === 0 && logs.length === 0 ? (
        <div className="flex h-full flex-col items-center justify-center space-y-2 text-center">
          <BsClockHistory className="h-8 w-8 text-gray-600 opacity-40 dark:text-gray-400" />
