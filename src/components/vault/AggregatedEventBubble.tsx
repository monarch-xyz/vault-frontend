        ...
        <div className="text-sm text-gray-800 dark:text-gray-200">
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {Array.isArray(markets) &&
              markets.map((marketId: string, index: number) => (
                <MarketSpan
                  key={`${marketId}-${index}`}
                  marketId={marketId}
                    ...
                    <div className="space-y-2 pl-2">
                      {eventList.map((event, idx) => (
                        <div
                          ...
                          className="flex items-center justify-between gap-2 border-b border-gray-100 pb-1 last:border-0 last:pb-0 dark:border-gray-700"
                        >
                          <div className="flex gap-4">
                            <div className="flex items-center gap-1">
                              <span className="font-medium">{formatAmount(event.amount)} USDC</span>
                              {event.sender && (
                              ...
                              {event.market_id && (
                                <MarketSpan
                                  ...
                                  market={allMarkets.find((m: any) =>
                                    isSameMarket(m.uniqueKey, event.market_id),
                                  )}
                                />
                              )}
                            </div>
                          ...
                          {event.tx_hash && (
                            <a
                              ...
                              className="font-zen text-xs text-secondary no-underline hover:underline"
                            >
